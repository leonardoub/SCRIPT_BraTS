...
def function_score_cv(data, labels, pipel, name_clf):
    ...
    for train_index, test_index in outer_kf.split(data, labels_encoded):
        ...
        # getting and saving the selected features chosen by anova_filter 

        p_values = pipel['red_dim'].pvalues_
        mask_percentile = pipel['red_dim'].get_support()
        ...
        outname = f'{name_clf}_features_selected_ANOVA_cv_score_outer_fold_{i}.csv'

        outdir = f'/home/users/ubaldi/TESI_BRATS/result_score/data_without_HISTO_SPATIAL/score_ROC_AUC_optimization/{name_clf}_features_selected_ANOVA'
